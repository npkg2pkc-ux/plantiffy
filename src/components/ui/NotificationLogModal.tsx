...
export function NotificationLogModal({
  isVisible,
  onClose,
  notification,
  ...
  useEffect(() => {
    if (isVisible && notification?.sheetName && notification?.recordId) {
      fetchLogs();
    }
  ...
  const fetchLogs = async () => {
    if (!notification?.sheetName || !notification?.recordId) return;

    setLoading(true);
    setError(null);

    try {
      const result = await getRecordActivityLogs(
